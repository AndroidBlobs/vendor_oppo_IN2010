...
function logObserver() {
    ...
    while [ x${boot_completed} != x"1" ];do
        echo "${CURTIME_FORMAT} log observer:device don't boot completed" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        sleep 10
        boot_completed=`getprop sys.boot_completed`
    done

    FreeSize=`df /data | grep /data | awk '{print $4}'`
    echo "${CURTIME_FORMAT} log observer:free size ${FreeSize}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        ...
        do
            if [ "" != "${ITEM_CONFIG}" ];then
                ...
                if [ -d ${SOURCE_PATH} ];then
                    ...
                    echo "${CURTIME_FORMAT} path: ${SOURCE_PATH}, ${TEMP_SIZE}/${LOG_COUNT_SIZE}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
                else
    ...
    echo "${CURTIME_FORMAT} log observer:data free and log size: ${FreeSize}|${LOG_COUNT_SIZE}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
}
...
function initOplusLog(){
    ...
    if [ "${PANICE_NABLE}" = "true" ] || [ x"${CAMERA_PANIC_ENABLE}" = x"true" ]; then
        ...
        echo "${CURTIME_FORMAT} init:start debug time: ${CURTIME}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
...
function tranferDump(){
    ...
    #wifi log
    tranferWifi
    echo "${CURTIME_FORMAT} transfer log:copy dump done" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
}
...
function tranferScreenshots(){
    ...
    echo "${CURTIME_FORMAT} transfer log:copy screenshots done" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
}
...
function transferColorOS(){
    ...
    echo "${CURTIME_FORMAT} transfer log:copy colorOS done" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
}

function checkSizeAndCopy(){
    LOG_SOURCE_PATH="$1"
    LOG_TARGET_PATH="$2"
    echo "${CURTIME_FORMAT} checksize and transfer:from ${LOG_SOURCE_PATH} to ${LOG_TARGET_PATH}"
    LIMIT_SIZE="10240"
    ...
    if [ -d "${LOG_SOURCE_PATH}" ]; then
        ...
        if [ ${TMP_LOG_SIZE} -le ${LIMIT_SIZE} ]; then  #log size less then 10M
            ...
            echo "${CURTIME_FORMAT} checkSize and transfer:${LOG_SOURCE_PATH} done" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        else
            echo "${CURTIME_FORMAT} checkSize and transfer:${LOG_SOURCE_PATH} SIZE:${TMP_LOG_SIZE}/${LIMIT_SIZE}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        fi
    fi
}
...
function checkNumberAndCopy(){
    LOG_SOURCE_PATH="$1"
    LOG_TARGET_PATH="$2"
    echo "${CURTIME_FORMAT} CHECKNUMBERANDCOPY:from ${LOG_SOURCE_PATH} to ${LOG_TARGET_PATH}"
    LIMIT_NUM=1000

    if [ -d "${LOG_SOURCE_PATH}" ] && [ ! "`ls -A ${LOG_SOURCE_PATH}`" = "" ]; then
        TMP_LOG_NUM=`ls -lR ${LOG_SOURCE_PATH} |grep "^-"|wc -l | awk '{print $1}'`
        echo "${CURTIME_FORMAT} CHECKNUMBERANDCOPY:${LOG_SOURCE_PATH} ${TMP_LOG_NUM}/${LIMIT_NUM}"
...
function checkNumberSizeAndMove(){
    ...
    LOG_LIMIT_NUM="$3"
    LOG_LIMIT_SIZE="$4"
    echo "${CURTIME_FORMAT} CHECKNUMBERSIZEANDMOVE:FROM ${LOG_SOURCE_PATH} TO ${LOG_TARGET_PATH}"
    LIMIT_NUM=1000
    #500*1024KB
    LIMIT_SIZE="512000"
...
function checkNumberAndMove(){
    ...
    if [ -d "${LOG_SOURCE_PATH}" ] && [ ! "`ls -A ${LOG_SOURCE_PATH}`" = "" ]; then
        ...
        if [ ${TMP_LOG_NUM} -le ${LIMIT_NUM} ]; then  #log number less then 1000
            ...
            mv ${LOG_SOURCE_PATH}/* ${LOG_TARGET_PATH}
            echo "${CURTIME_FORMAT} checkNumber and move:${LOG_SOURCE_PATH} done" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        else
            echo "${CURTIME_FORMAT} checkNumber and move:${LOG_SOURCE_PATH} NUM:${TMP_LOG_NUM}/${LIMIT_NUM}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        fi
    fi
}
...
function tranferThirdApp(){
    ...
    echo "${CURTIME_FORMAT} transfer log:copy third app done" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
}
...
function tranferPower(){
    ...
    echo "${CURTIME_FORMAT} transfer log:copy power done" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
}
    ...
    if [ -d  ${DATA_LOG_PATH} ]; then
        ...
        for SUB_DIR in ${ALL_SUB_DIR};do
            if [ -d ${DATA_LOG_PATH}/${SUB_DIR} ] || [ -f ${DATA_LOG_PATH}/${SUB_DIR} ]; then
                echo "${CURTIME_FORMAT} ${LOGTAG}:mv ${DATA_LOG_PATH}/${SUB_DIR} to ${newpath}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
...
function transferDataOppoLog(){
    ...
    chmod 770 ${DATA_OPLUS_LOG}/ -R
    #tar -czvf ${newpath}/LOG.dat.gz -C /data/oppo/log .

    #checkNumberAndCopy "/data/oppo/log" "${newpath}/log"
    ...
    if [ -d  ${DATA_OPLUS_LOG} ]; then
        ...
        for SUB_DIR in ${ALL_SUB_DIR};do
            if [ -d ${DATA_OPLUS_LOG}/${SUB_DIR} ] || [ -f ${DATA_OPLUS_LOG}/${SUB_DIR} ]; then
                checkNumberAndCopy "${DATA_OPLUS_LOG}/${SUB_DIR}" "${newpath}/log/${SUB_DIR}"
            fi
        done
    fi
...
function transfer2SDCard(){
    ...
    echo "${CURTIME_FORMAT} transfer log:done...." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
}

function transfer_log(){
    echo "${CURTIME_FORMAT} transfer:start...." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
    ...
    echo "${CURTIME_FORMAT} transfer:done...." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        ...
        do
            if [ "" != "${item_config}" ];then
                ...
                if [ -d ${SOURCE_PATH} ];then
                    ...
                    echo "${CURTIME_FORMAT} transfer log path: ${OPERATION} ${SOURCE_PATH} done, size ${TEMP_SIZE}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
                else
                    echo "${CURTIME_FORMAT} transfer log path: ${SOURCE_PATH}, No such file or directory" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
                fi
            fi
        done
    else
...
function initLogSizeAndNums() {
    ...
    echo "${CURTIME_FORMAT} init:data FreeSize:${FreeSize} and GSIZE:${GSIZE}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
    ...
    echo "${CURTIME_FORMAT} init:main=${tmpMain}, radio=${tmpRadio}, event=${tmpEvent}, kernel=${tmpKernel}, tcpdump=${tmpTcpdump}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
    ...
    if [ ${FreeSize} -ge ${GSIZE} ]; then
        if [ "${tmpMain}" != "" ]; then
            ...
            echo "${CURTIME_FORMAT} init:tmpAndroidSize=${tmpAndroidSize}, tmpAdnroidCount=${tmpAdnroidCount}, androidSize=${androidSize}, androidCount=${androidCount}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
            if [ ${androidCount} -ge ${tmpAdnroidCount} ]; then
                androidCount=${tmpAdnroidCount}
            fi
            echo "${CURTIME_FORMAT} init:last androidCount=${androidCount}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        fi
...
function logcatMain(){
    ...
    echo "${CURTIME_FORMAT} logcat main:path=${DATA_LOG_APPS_PATH}, size=${androidSize}, Nums=${androidCount}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
...
function getSystemStatus() {
    echo "${CURTIME_FORMAT} dumpSystem:start...." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
    boot_completed=`getprop sys.boot_completed`
    if [ x${boot_completed} == x"1" ]
    then
        outputPath="${DATA_LOG_PATH}/SI_stop"

        echo "${CURTIME_FORMAT} dumpSystem:${outputPath}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        mkdir -p ${outputPath}
        ...
        echo "${CURTIME_FORMAT} dumpSystem:ps,top" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        ...
        echo "${CURTIME_FORMAT} dumpSystem:getprop" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        getprop > ${outputPath}/prop.txt
        echo "${CURTIME_FORMAT} dumpSystem:df" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        df > ${outputPath}/df.txt
        echo "${CURTIME_FORMAT} dumpSystem:lpdump" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        lpdump > ${outputPath}/lpdump.txt
        echo "${CURTIME_FORMAT} dumpSystem:mount" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        mount > ${outputPath}/mount.txt
        setprop sys.tranfer.finished mv:mount
        echo "${CURTIME_FORMAT} dumpSystem:cat" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        cat data/system/packages.xml  > ${outputPath}/packages.txt
        cat data/system/appops.xml  > ${outputPath}/appops.xml
        echo "${CURTIME_FORMAT} dumpSystem:dumpsys appops" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        ...
        echo "${CURTIME_FORMAT} dumpSystem:user" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        ...
        echo "${CURTIME_FORMAT} dumpSystem:dropbox" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
        ...
        echo "${CURTIME_FORMAT} dumpSystem:done...." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
    fi
}

#Zhiming.chen@ANDROID.DEBUG 2724830, 2020/01/04,
function getMemoryMap() {
    echo "${CURTIME_FORMAT} dumpSystem:memory map start...." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
    LI=0
    LMI=4
    LMM=0
    MEMORY=921600
    ...
    echo "${CURTIME_FORMAT} dumpSystem:memory map done...." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
}

#Chunbo.Gao@ANDROID.DEBUG 2020/6/18, Add for wechat
function dumpWechatInfo() {
    echo "${CURTIME_FORMAT} dumpWechatInfo:start...." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
    wechatPath="${DATA_LOG_PATH}/SI_stop/wechat"
    echo "${CURTIME_FORMAT} dumpWechatInfo:${wechatPath}"
    echo "${CURTIME_FORMAT} dumpWechatInfo:${wechatPath}" >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
    mkdir -p ${wechatPath}
    ...
    touch ${wechatPath}/finish_weixin
    echo "${CURTIME_FORMAT} dumpWechatInfo:done...." >> ${SDCARD_LOG_BASE_PATH}/logkit_transfer.log
}
...
function oidtlogs() {
    ...
    cp system/system_ext/etc/sys_stamp_config.xml sdcard/OppoStamp/config/
